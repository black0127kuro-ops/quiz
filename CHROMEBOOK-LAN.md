...
4. **他の端末から繋ぐ設定**（OS によって名称が違います）

### パターンA: 「ポート転送」がある（新しい ChromeOS）

1. **設定** → **開発者向け** → **Linux 開発環境**
2. **ポート転送**（Port forwarding）を開く  
   ※ 設定の検索ボックスに **`ポート転送`** や **`port forwarding`** と入力すると見つかることがあります
3. **追加** → ポート **`3000`**、種類 **TCP**、名前は任意（例: Quiz）
4. 一覧で **オン** にする（Linux ターミナルが一度起動している必要あり）

### パターンB: 「ローカルネットワークからの接続を許可」がある（古い UI）

そのスイッチを **オン** にする。

### どちらも無い場合

学校管理の Chromebook では **LAN 公開が禁止** されていることがあります。  
→ **Chromebook は参加者だけ** にして、主催は **Windows + USB 版**（`quiz-lan.bat`）を検討してください。

※ ポート転送は **再起動後オフ** になることがあります。クイズの前に毎回オンを確認してください。
...
| 参加者だけ繋がらない | **ポート転送** で TCP **3000** を追加してオン。Linux ターミナルが起動中か確認 |
| ポート転送自体が無い | 学校管理の可能性 → Windows/USB 主催へ |
